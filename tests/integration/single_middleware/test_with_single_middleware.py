...
def test_middleware_called(webapi, telegraf_client):
    ...
    verify(telegraf_client, times=1).metric('hits-/a', values={'hits': 1}, tags={'method': 'GET', 'success': 'True', 'status': '200 OK'})
...
def test_middleware_called_n_times(webapi, telegraf_client):
    ...
    assert webapi.get('/a').text == 'Success'
    assert webapi.get('/a').text == 'Success'
    verify(telegraf_client, times=3).metric('hits-/a', values={'hits': 1}, tags={'method': 'GET', 'success': 'True', 'status': '200 OK'})
...
def test_middleware_called_params(webapi, telegraf_client):
    ...
    verify(telegraf_client, times=1).metric('hits-/c/{id}',
                                            values={'hits': 1},
                                            tags={'id': '1', 'method': 'GET', 'success': 'True', 'status': '200 OK'})
